...
export default function Wizard({ onComplete }) {
	...
	// Load presets for preset selection step
	const presetItems = useMemo(() => {
		try {
			...
			const items = [
				{ label: '⚙️  No preset (use profile config)', value: 'none' }
			];
			...
			return [{ label: '⚙️  No preset (use profile config)', value: 'none' }];
		}
	}, []);

	// Token budget options
	const tokenBudgetItems = [
		{ label: '∞  No limit (include everything)', value: 'none' },
		{ label: '📊 50k tokens (small context)', value: '50000' },
		{ label: '📊 100k tokens (medium context)', value: '100000' },
		{ label: '📊 200k tokens (large context)', value: '200000' },
		{ label: '📊 500k tokens (very large context)', value: '500000' },
		{ label: '📊 1M tokens (maximum context)', value: '1000000' }
	];

	// Rule tracing options
	const ruleTracingItems = [
		{ label: '❌ Disabled (faster)', value: 'false' },
		{ label: '✅ Enabled (debug filters)', value: 'true' }
	];
	...
	const handlePresetSelect = useCallback((item) => {
		setAnswers(prev => ({ ...prev, preset: item.value }));
		setStep('token-budget');
	}, []);

	const handleTokenBudgetSelect = useCallback((item) => {
		setAnswers(prev => ({ ...prev, tokenBudget: item.value }));
		setStep('rule-tracing');
	}, []);

	const handleRuleTracingSelect = useCallback((item) => {
		setAnswers(prev => ({ ...prev, ruleTracing: item.value }));
	...
	if (step === 'preset') {
		children.push(
			React.createElement(Box, { key: 'preset-step', flexDirection: 'column' },
				React.createElement(Box, { key: 'question-box-3', marginBottom: 1 },
					React.createElement(Text, null, 'Apply a preset?')
				),
				React.createElement(SelectInput, {
					key: 'select-input-3',
					items: presetItems,
	...
	// Step 4: Token Budget
	if (step === 'token-budget') {
		children.push(
			React.createElement(Box, { key: 'token-budget-step', flexDirection: 'column' },
				React.createElement(Box, { key: 'question-box-4', marginBottom: 1 },
					React.createElement(Text, null, 'Set token budget?')
				),
				React.createElement(SelectInput, {
					key: 'select-input-4',
					items: tokenBudgetItems,
					onSelect: handleTokenBudgetSelect
				}),
				React.createElement(Box, { key: 'help-box-4', marginTop: 1 },
					React.createElement(Text, { dimColor: true }, '[↑↓] Navigate  [Enter] Select  [Esc] Cancel')
				)
			)
		);
	...
	// Step 5: Rule Tracing
	if (step === 'rule-tracing') {
		children.push(
			React.createElement(Box, { key: 'rule-tracing-step', flexDirection: 'column' },
				React.createElement(Box, { key: 'question-box-5', marginBottom: 1 },
					React.createElement(Text, null, 'Enable rule tracing?')
				),
				React.createElement(SelectInput, {
					key: 'select-input-5',
					items: ruleTracingItems,
					onSelect: handleRuleTracingSelect
				}),
				React.createElement(Box, { key: 'help-box-5', marginTop: 1 },
					React.createElement(Text, { dimColor: true }, '[↑↓] Navigate  [Enter] Select  [Esc] Cancel')
				)
			)
		);
	...
	if (step === 'output-format') {
		children.push(
			React.createElement(Box, { key: 'output-format-step', flexDirection: 'column' },
				React.createElement(Box, { key: 'question-box-6', marginBottom: 1 },
				...
				React.createElement(Box, { key: 'help-box-6', marginTop: 1 },
					React.createElement(Text, { dimColor: true }, '[↑↓] Navigate  [Enter] Select  [Esc] Cancel')
				)
			)
		);
	...
	if (step === 'complete') {
		const completeBoxChildren = [
			...
			React.createElement(Box, { key: 'complete-info', flexDirection: 'column', marginBottom: 1 },
				...
				React.createElement(Text, null, `Preset: ${answers.preset || 'none'}`),
				React.createElement(Text, null, `Token Budget: ${answers.tokenBudget === 'none' ? 'unlimited' : answers.tokenBudget}`),
				React.createElement(Text, null, `Rule Tracing: ${answers.ruleTracing === 'true' ? 'enabled' : 'disabled'}`),
