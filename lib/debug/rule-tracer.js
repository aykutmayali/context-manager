...
export class RuleTracer {
  ...
  constructor(options = {}) {
    ...
    this.appliedPreset = null; // Track applied preset
    this.fittingDecisions = []; // Track token budget fitting decisions
  }
  ...
  recordFileDecision(file, decision) {
    ...
    if (!file || typeof file !== 'string') {
      logger.warn('Invalid file path provided to recordFileDecision');
      return;
    }

    if (!decision || typeof decision !== 'object') {
      logger.warn('Invalid decision object provided to recordFileDecision');
      return;
    }
   ...
   * @param {Object} preset - Preset that was applied
   */
  recordPresetApplication(preset) {
    if (!this.options.enabled) return;
    ...
    logger.info(`Recorded preset application: ${preset.name}`);
  }

  /**
   * Record token budget fitting decision
   * @param {Object} decision - Fitting decision details
   */
  recordFittingDecision(decision) {
    if (!this.options.enabled) return;

    this.fittingDecisions.push({
      timestamp: new Date().toISOString(),
      ...decision
    });

    logger.debug(`Recorded fitting decision: ${decision.type}`);
  }
  ...
  generateReport() {
    ...
    // Preset information (if applied)
    if (this.appliedPreset) {
      lines.push('🎯 Applied Preset:');
      const icon = this.appliedPreset.icon || '📦';
      lines.push(`   ${icon} ${this.appliedPreset.name} (${this.appliedPreset.id})`);
      lines.push(`   ${this.appliedPreset.description}`);
      
      if (this.appliedPreset.metadata && this.appliedPreset.metadata.bestFor) {
        lines.push(`   Best for: ${this.appliedPreset.metadata.bestFor.join(', ')}`);
      }

      // Show preset filters
      const filters = this.appliedPreset.filters;
      if (filters.include && filters.include.length > 0) {
        lines.push(`   Include patterns: ${filters.include.length}`);
      }
      if (filters.exclude && filters.exclude.length > 0) {
        lines.push(`   Exclude patterns: ${filters.exclude.length}`);
      }
      if (filters.methodInclude && filters.methodInclude.length > 0) {
        lines.push(`   Method include patterns: ${filters.methodInclude.length}`);
      }
      if (filters.methodExclude && filters.methodExclude.length > 0) {
        lines.push(`   Method exclude patterns: ${filters.methodExclude.length}`);
      }

      lines.push('');
    }

    // Token budget fitting information (if applied)
    if (this.fittingDecisions.length > 0) {
      lines.push('💰 Token Budget Fitting:');
      for (const decision of this.fittingDecisions) {
        if (decision.type === 'strategy') {
          lines.push(`   Strategy: ${decision.strategy}`);
          lines.push(`   Target: ${decision.targetTokens} tokens`);
          lines.push(`   Result: ${decision.actualTokens} tokens`);
          lines.push(`   Reduction: ${decision.reduction} tokens (${decision.reductionPercent.toFixed(1)}%)`);
        } else if (decision.type === 'file-excluded') {
          lines.push(`   ❌ Excluded: ${decision.file} (${decision.reason})`);
          if (decision.importance !== undefined) {
            lines.push(`      Importance: ${decision.importance.toFixed(1)}`);
          }
        } else if (decision.type === 'file-included') {
          lines.push(`   ✅ Included: ${decision.file} (${decision.reason})`);
          if (decision.importance !== undefined) {
            lines.push(`      Importance: ${decision.importance.toFixed(1)}`);
          }
        }
  ...
  exportJSON() {
    ...
    return {
      ...
      fittingDecisions: this.fittingDecisions,
