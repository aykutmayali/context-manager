...
export class TokenBudgetFitter {
  ...
  fitToWindow(files, options = {}) {
    ...
    if (!Array.isArray(files)) {
      throw new TokenBudgetError('Files must be an array');
    }

    if (files.length === 0) {
      logger.warn('No files to fit');
      return {
        files: [],
        totalTokens: 0,
        originalTokens: 0,
    ...
    if (this.targetTokens <= 0) {
      throw new TokenBudgetError('Target tokens must be greater than 0');
    }
    ...
    // Record fitting decisions in tracer if provided
    if (opts.ruleTracer && opts.ruleTracer.isEnabled()) {
      this.recordFittingInTracer(opts.ruleTracer, result, totalTokens, reduction, reductionPercent);
    }
  ...
  /**
   * Record fitting decisions in rule tracer
   * @param {Object} ruleTracer - RuleTracer instance
   * @param {Object} result - Fitting result
   * @param {number} totalTokens - Total tokens after fitting
   * @param {number} reduction - Token reduction
   * @param {number} reductionPercent - Reduction percentage
   * @private
   */
  recordFittingInTracer(ruleTracer, result, totalTokens, reduction, reductionPercent) {
    // Record overall strategy decision
    ruleTracer.recordFittingDecision({
      type: 'strategy',
      strategy: result.strategy,
    ...
    // Record included files with importance scores
    for (const file of result.files) {
      ruleTracer.recordFittingDecision({
        type: 'file-included',
        file: file.relativePath || file.path,
        reason: file.isEntryPoint ? 'Entry point (preserved)' : 'High importance',
        importance: file.importance,
        tokens: file.tokens
      });
    }

    // Record excluded files with reasons
    for (const file of result.excluded) {
      ruleTracer.recordFittingDecision({
        type: 'file-excluded',
        file: file.relativePath || file.path,
        reason: this.getExclusionReason(file),
        importance: file.importance,
        tokens: file.tokens
      });
    }
  ...
  /**
   * Get exclusion reason for a file
   * @param {Object} file - File object
   * @returns {string} Exclusion reason
   * @private
   */
  getExclusionReason(file) {
    if (file.importance < 30) {
      return 'Low importance score';
    }
    if (file.relativePath && file.relativePath.includes('/test/')) {
      return 'Test file';
    }
    if (file.relativePath && file.relativePath.includes('/docs/')) {
      return 'Documentation file';
    }
    if (file.extension === '.md') {
      return 'Markdown documentation';
    }
    return 'Exceeded token budget';
  }
  ...
  recommendStrategy(files, targetTokens) {
    ...
    // Calculate percent over budget relative to target (not total)
    // e.g., 160k/100k = 60% over, 200k/100k = 100% over
    const overBudgetPercent = (overBudget / targetTokens) * 100;
  ...
  calculateImportanceScores(files, options) {
    for (const file of files) {
      // Skip if already calculated (cached)
      if (file.importance === undefined) {
