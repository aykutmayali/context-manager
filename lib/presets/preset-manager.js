...
export class PresetManager {
  ...
  loadPresets() {
    ...
    try {
      if (!fs.existsSync(this.presetsPath)) {
        ...
        this.loaded = true; // Mark as loaded to avoid repeated file checks
        return [];
      }
   ...
   * @param {Object} [ruleTracer] - Optional RuleTracer instance for tracking
   * @returns {AppliedPreset} Applied preset information
   */
  applyPreset(name, projectRoot, ruleTracer = null) {
    ...
    // Validate inputs after checking preset exists
    if (!projectRoot || typeof projectRoot !== 'string') {
      throw new Error('Project root must be a non-empty string');
    }
    ...
    // Record preset application in tracer if provided
    if (ruleTracer && ruleTracer.isEnabled()) {
      ruleTracer.recordPresetApplication(preset);
    }
    ...
    try {
      ...
      if (preset.filters.include && preset.filters.include.length > 0) {
        ...
        const includeContent = preset.filters.include.join('\n');
        fs.writeFileSync(includePath, includeContent, 'utf-8');
        ...
        // Write directly instead of copy for better performance
        fs.writeFileSync(activeIncludePath, includeContent, 'utf-8');
      ...
      if (preset.filters.exclude && preset.filters.exclude.length > 0) {
        ...
        const excludeContent = preset.filters.exclude.join('\n');
        ...
        // Write directly instead of copy for better performance
        fs.writeFileSync(activeExcludePath, excludeContent, 'utf-8');
      ...
      if (preset.filters.methodInclude && preset.filters.methodInclude.length > 0) {
        ...
        const methodIncludeContent = preset.filters.methodInclude.join('\n');
        fs.writeFileSync(methodIncludePath, methodIncludeContent, 'utf-8');
        ...
        // Write directly instead of copy for better performance
        fs.writeFileSync(activeMethodIncludePath, methodIncludeContent, 'utf-8');
      ...
      if (preset.filters.methodExclude && preset.filters.methodExclude.length > 0) {
        ...
        const methodExcludeContent = preset.filters.methodExclude.join('\n');
        fs.writeFileSync(methodExcludePath, methodExcludeContent, 'utf-8');
        ...
        // Write directly instead of copy for better performance
        fs.writeFileSync(activeMethodExcludePath, methodExcludeContent, 'utf-8');
